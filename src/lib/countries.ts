...
// Dynamically generate flag emojis from ISO-2 country codes
...
const rawCountries = [
  ...
  { name: "Congo, Democratic Republic of", code: "CD", dial_code: "+243" },
  { name: "Congo, Republic of", code: "CG", dial_code: "+242" },
  ...
  { name: "Saint Vincent", code: "VC", dial_code: "+1-784" },
...
const defaultRule: PhoneRule = { minLength: 7, maxLength: 15, placeholder: "Enter phone number" };

const phoneRules: Record<string, PhoneRule> = {
  IN: { minLength: 10, maxLength: 10, placeholder: "9876543210" },
  US: { minLength: 10, maxLength: 10, placeholder: "2015550123" },
  CA: { minLength: 10, maxLength: 10, placeholder: "2015550123" },
  GB: { minLength: 10, maxLength: 11, placeholder: "7911123456" },
  AE: { minLength: 9, maxLength: 9, placeholder: "501234567" },
  SA: { minLength: 9, maxLength: 9, placeholder: "501234567" },
  AU: { minLength: 9, maxLength: 9, placeholder: "412345678" },
  SG: { minLength: 8, maxLength: 8, placeholder: "81234567" },
  DE: { minLength: 10, maxLength: 11, placeholder: "1701234567" },
  FR: { minLength: 9, maxLength: 10, placeholder: "612345678" },
  JP: { minLength: 10, maxLength: 10, placeholder: "9012345678" },
  CN: { minLength: 11, maxLength: 11, placeholder: "13912345678" }
};

export function getPhoneRule(countryCode: string): PhoneRule {
  if (!countryCode) return defaultRule;
  // Match by code like IN, US, or by dial prefix if that's passed
  let cleanCode = countryCode.toUpperCase();
  if (cleanCode.startsWith("+")) {
    const matched = rawCountries.find(c => c.dial_code === countryCode);
    if (matched) cleanCode = matched.code.toUpperCase();
  }
  return phoneRules[cleanCode] || defaultRule;
}
