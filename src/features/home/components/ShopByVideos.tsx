"use client";
import { ArrowRight, VolumeX, Sparkles, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <section className="py-10 md:py-12 relative overflow-hidden section-glass-ambient ambient-gold border-b border-bronze-500/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative">
          ...
          <button
            onClick={() => scroll("left")}
            className="absolute -left-4 sm:-left-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-surface-card/90 dark:bg-surface-card/85 hover:bg-surface-hover text-heading shadow-xl border border-bronze-500/30 hidden md:flex items-center justify-center"
            aria-label="Scroll left"
          >
          ...
          >
            {videos.map((video) => (
              <div
                key={video.id}
                className="shrink-0 w-[280px] sm:w-[320px] lg:w-[calc(25%-18px)] flex flex-col justify-between bg-surface-card border border-bronze-500/[0.14] rounded-3xl overflow-hidden shadow-sm"
              >
                ...
                <div className="relative aspect-[9/16] w-full overflow-hidden bg-black group">
                  ...
                  <div className="absolute top-4 right-4 z-20 w-8 h-8 rounded-full bg-black/40 flex items-center justify-center text-white/90">
                    <VolumeX size={14} />
                  </div>
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-5 z-20 text-white space-y-4">
                    ...
                    <Link
                      ...
                      className="group/btn inline-flex items-center justify-center gap-1.5 px-4 py-2.5 w-full rounded-full bg-gradient-to-r from-bronze-500 to-bronze-600 hover:from-bronze-400 hover:to-bronze-500 text-white font-semibold shadow-md text-xs"
                    >
                      Inquire Product
                      <ArrowRight size={13} />
                    </Link>
          ...
          <button
            onClick={() => scroll("right")}
            className="absolute -right-4 sm:-right-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-surface-card/90 dark:bg-surface-card/85 hover:bg-surface-hover text-heading shadow-xl border border-bronze-500/30 hidden md:flex items-center justify-center"
            aria-label="Scroll right"
          >
